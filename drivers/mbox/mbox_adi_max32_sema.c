...
#define DT_DRV_COMPAT adi_mbox_max32_semaphore
...
#define MAX32_SEMA_IRQ_EN     BIT(MAX32_SEMA_IRQ_EN_BIT)

#define MAX32_SEMA_IRQ_SET_BIT 16
#define MAX32_SEMA_IRQ_SET     BIT(MAX32_SEMA_IRQ_SET_BIT)
...
struct mbox_adi_max32_sema_data {
	mbox_callback_t cb;
	void *cb_user_data;
	mbox_channel_id_t enabled_channel;
};

static inline mem_addr_t get_irq_reg(const struct device *dev, mbox_channel_id_t channel_id)
{
	const struct mbox_adi_max32_sema_config *cfg = dev->config;

	switch (channel_id) {
	case 0:
		return (mem_addr_t)&cfg->regs->irq0;
	case 1:
		return (mem_addr_t)&cfg->regs->irq1;
	default:
...
static void mbox_adi_max32_sema_isr(const struct device *dev)
{
	...
	mem_addr_t irq_reg = get_irq_reg(dev, data->enabled_channel);

	if (!irq_reg) {
		LOG_ERR("ISR with no enabled IRQ");
		return;
	}
	...
	if (data->cb) {
		...
		data->cb(dev, data->enabled_channel, data->cb_user_data, mbox_data_arg);
	} else {
						 ...
						 void *user_data)
{
	...
	__ASSERT(channel_id < 2, "Two channels supported by the MAX32 mbox");
					   ...
					   bool enabled)
{
	const struct mbox_adi_max32_sema_config *cfg = dev->config;
	struct mbox_adi_max32_sema_data *data = dev->data;
	mem_addr_t irq_reg = get_irq_reg(dev, channel_id);
	...
	if (enabled) {
		data->enabled_channel = channel_id;
		irq_enable(cfg->irq_num);
		sys_set_bit(irq_reg, MAX32_SEMA_IRQ_EN_BIT);
	} else {
		data->enabled_channel = UINT8_MAX;
				    ...
				    const struct mbox_msg *msg)
{
	...
	sys_set_bits(irq_reg, (MAX32_SEMA_IRQ_SET | MAX32_SEMA_IRQ_EN));
